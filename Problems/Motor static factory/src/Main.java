...
class MotorStaticFactory {
    ...
    public static Motor make(char type, String model, long power) {
        ...
        if (type == 'H' || type == 'h') return new HydraulicMotor(model, power);
        if (type == 'E' || type == 'e') return new ElectricMotor(model, power);
        if (type == 'W' || type == 'w') return new WarpDrive(model, power);
        if (type == 'P' || type == 'p') return new PneumaticMotor(model, power);
        return null;
    }
}
